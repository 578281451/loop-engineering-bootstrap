...
FILES = {
    ...
    ".agent/execution-gates.md": "# Execution Gates\n\nEvery executable user request is a Task, including service startup, log inspection, tests, diagnosis, code, documentation, and deployment preparation. Debug mode is never a protocol bypass. Create or resume a Task before commands. S0 is a fast path with Task, evidence, state, Event, run log, and structured report.\n\nS2/S3 delegation is mandatory. If the host cannot delegate, record blocked_delegation and do not claim orchestration or independent verification. A task is complete only after Task status, Event, run log, state, evidence, and the structured final report exist.\n",
    ".agent/final-report-contract.md": "# Final Report Contract\n\nEvery executable task report must include: Task ID, Attempt ID, status, route, actual execution mode, delegation requested/child tasks/worker results, verifier verdict, root cause or runtime diagnosis, changed files or actions, commands, evidence, state/Event/run-log records, unfinished work, and next action. Missing evidence must be stated; never claim a child Agent, verifier, or E2E run without records.\n",
    ".agent/routing.yaml": "version: 1\ndefault_level: S0\nlevels: [S0, S1, S2, S3]\nmax_child_agents: 3\nrequire_independent_verifier_for: [S2, S3]\nrequire_human_gate_for: [S3]\n",
    ".agent/constitution.md": """# Loop Engineering Constitution\n\nThis file contains portable process rules only. Project facts remain in project documents.\n\n## Rules\n\n- Every executable user request is a Task, including service startup, log inspection, test execution, diagnosis, code, documentation, and deployment preparation.\n- Create or resume the Task and route it before runtime or repository mutation. Debug mode is never a protocol bypass.\n- Plan before mutation and define testable acceptance.\n- A normal user-requested Bug or feature starts the complete interactive loop automatically.\n- L1/report_only blocks unattended mutation, merge, push, and deployment; it permits implementation in the explicitly requested interactive conversation.\n- S2/S3 delegation is mandatory; if unavailable, record blocked_delegation and do not claim orchestration.\n- A task is not complete until Task, Event, run log, state, evidence, and structured final report are recorded.\n- Separate observed facts, assumptions, and recommendations.\n- Match verification and review to risk; S2/S3 requires an independent verifier.\n- Load the smallest sufficient context through the knowledge index.\n- Change protected knowledge only through Event -> Patch -> Validate -> Review -> Apply -> Audit.\n- Preserve superseded knowledge and never silently overwrite it.\n- User-facing frontend workflows require browser E2E evidence or a reviewed, time-limited exception.\n""",
